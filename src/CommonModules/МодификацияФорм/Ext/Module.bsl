...
Процедура МодифицироватьФорму(Форма, МакетОписанияМодификаций, ГенераторПротокола = Неопределено) Экспорт
...
Функция ЭтоСтрокаРеквизита(Идентификатор)
...
Функция ЭтоСтрокаТипаРеквизита(Идентификатор)
	Идентификаторы = Новый Массив;
...
Функция ЭтоСтрокаЭлемента(Идентификатор)
	Идентификаторы = Новый Массив;
...
Функция ЭтоСтрокаСвойстваЭлемента(Идентификатор)
	Идентификаторы = Новый Массив;
...
Функция ЭтоДекорацияФормы(Идентификатор)
	Идентификаторы = Новый Массив;
...
Функция ЭтоГруппаФормы(Идентификатор)
	Идентификаторы = Новый Массив;
...
Функция ЭтоКнопкаФормы(Идентификатор)
	Идентификаторы = Новый Массив;
...
Функция ЭтоТаблицаФормы(Идентификатор)
	Идентификаторы = Новый Массив;
...
Функция ЭтоПолеФормы(Идентификатор)
	Идентификаторы = Новый Массив;
...
Функция ОписаниеПолей(СтрокаДокумента, НомерСтроки)
	РазделительПолей = МодификацияФормПереопределеяемый.РазделительПолей();
...
Процедура ОбработатьПоля(ОписаниеМодификаций, ОписаниеПолей, ГенераторПротокола)
	...
	Если ЭтоСтрокаРеквизита(Идентификатор) Тогда
		ОбработатьСтрокуРеквизита(ОписаниеМодификаций, ОписаниеПолей, ГенераторПротокола);
	ИначеЕсли ЭтоСтрокаТипаРеквизита(Идентификатор) Тогда
		ОбработатьСтрокуТипаРеквизита(ОписаниеМодификаций, ОписаниеПолей, ГенераторПротокола);
	ИначеЕсли ЭтоСтрокаЭлемента(Идентификатор) Тогда
		ОбработатьСтрокуЭлемента(ОписаниеМодификаций, ОписаниеПолей, ГенераторПротокола);
	ИначеЕсли ЭтоСтрокаСвойстваЭлемента(Идентификатор) Тогда
//		ОбработатьСтрокуПараметраЭлемента(ОписаниеМодификаций, Поля, ГенераторПротокола);
	Иначе
		Возврат;
	КонецЕсли;
...
Процедура ОбработатьСтрокуЭлемента(ОписаниеМодификаций, ОписаниеПолей, ГенераторПротокола)
	...
	Если КоличествоПолей > 3 Тогда
		ИмяРодителя = ВРег(ОписаниеПолей.Поля[3]);
	КонецЕсли;
	Если КоличествоПолей > 4 Тогда
		ИмяПоследующегоЭлемента = ВРег(ОписаниеПолей.Поля[4]);
	КонецЕсли;
	Попытка
	...
	ОписаниеЭлемента.Вставить("СвойстваЭлемента", Новый Соответствие);
...
Функция ТипЭлемента(ИмяТипаЭлемента)
	Если ВРег(ИмяТипаЭлемента) = ЭтоДекорацияФормы(ВРег(ИмяТипаЭлемента)) Тогда
		Возврат Тип("ДекорацияФормы");
	ИначеЕсли ВРег(ИмяТипаЭлемента) = ЭтоГруппаФормы(ВРег(ИмяТипаЭлемента)) Тогда
		Возврат Тип("ГруппаФормы");
	ИначеЕсли ВРег(ИмяТипаЭлемента) = ЭтоКнопкаФормы(ВРег(ИмяТипаЭлемента)) Тогда
		Возврат Тип("КнопкаФормы");
	ИначеЕсли ВРег(ИмяТипаЭлемента) = ЭтоТаблицаФормы(ВРег(ИмяТипаЭлемента)) Тогда
		Возврат Тип("ТаблицаФормы");
	ИначеЕсли ВРег(ИмяТипаЭлемента) = ЭтоПолеФормы(ВРег(ИмяТипаЭлемента)) Тогда
