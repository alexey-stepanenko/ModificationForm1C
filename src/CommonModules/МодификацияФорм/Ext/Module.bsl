...
Процедура МодифицироватьФорму(Форма, МакетОписаниеМодификаций, ОбработчикОшибок = Неопределено) Экспорт
	Если ОбработчикОшибок = Неопределено Тогда
		ОбработчикОшибок = Обработки.ОбработчикОшибокМодификацииФормНаЭкран.СоздатьОбработчикОшибок(Истина);
	КонецЕсли;
	
	ОписаниеМодификаций = ОписаниеМодификаций(МакетОписаниеМодификаций, ОбработчикОшибок);
	МодифицироватьРеквизиты(Форма, ОписаниеМодификаций, ОбработчикОшибок);
...
Функция ОписаниеМодификаций(МакетОписаниеМодификаций, ОбработчикОшибок)
	...
	Для Сч = 1 По МакетОписаниеМодификаций.КоличествоСтрок() Цикл
		...
		Попытка
			ОбработатьПоля(РезультатФункции, ПоляИзСтроки, ОбработчикОшибок);
		Исключение
			...
			ОбработчикОшибок.ОбработатьОшибку(ОписаниеОшибки);
			Если ОбработчикОшибок.ПрерватьМодификациюПриОшибке() Тогда
				ВызватьИсключение;
			КонецЕсли;	
		КонецПопытки;
	КонецЦикла;
...
Процедура ОбработатьПоля(ОписаниеМодификаций, Поля, ОбработчикОшибок)
	...
	Если Идентификатор = СтрокаРеквизита() Тогда
		ОбработатьСтрокуРеквизита(ОписаниеМодификаций, Поля, ОбработчикОшибок);
	ИначеЕсли Идентификатор = СтрокаТипаРеквизита() Тогда
		ОбработатьСтрокуТипаРеквизита(ОписаниеМодификаций, Поля, ОбработчикОшибок);
	Иначе
		Возврат;
	КонецЕсли;
...
Процедура ОбработатьСтрокуРеквизита(ОписаниеМодификаций, Поля, ОбработчикОшибок)
	ИмяРеквизита = Поля[1];
...
Процедура ОбработатьСтрокуТипаРеквизита(ОписаниеМодификаций, Поля, ОбработчикОшибок)
